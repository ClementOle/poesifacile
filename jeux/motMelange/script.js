...
function afficheRes(res) {
    ...
    if(res === true) {
        p.innerHTML = 'Bravo';
        boutonSuite.classList.remove("cacher");
        boutonV.classList.add("cacher");
    } else {
        p.innerHTML = 'Perdu';
    }
}
...
boutonSuite.addEventListener('click', function(event){
    event.preventDefault();
    event.stopPropagation();
    sessionStorage.setItem("LevelReussi", '1');
    window.location.href = "http://localhost/poesifacile/jeux/motMelange/motMelange2.php";
});

boutonAbandon.addEventListener('click', function(event){
    event.preventDefault();
    event.stopPropagation();
    sessionStorage.setItem("LevelReussi", '0');
    window.location.href = "http://localhost/poesifacile/jeux/motMelange/motMelange2.php";
})
...
var tableau1 = ['SELECT', '*', 'FROM', 'joueur'];
var tableauVide = new Array(tableau1.length);
tableauVide.fill("");
var reponse1 = ['SELECT', '*', 'FROM', 'joueur'];
