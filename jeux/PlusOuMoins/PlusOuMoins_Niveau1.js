...
if (isNaN(nombreJoueur)){
	...
	while((nombreJoueur != nombreAleatoire ) && (nbTentatives < 6) && estUnEntier == true){
		...
		if((nombreJoueur == nombreAleatoire) && (estUnEntier == true)){
			...
			sessionStorage.setItem("pointsGagnesNiveau1", 1);
			sessionStorage.setItem("pointsPerdusNiveau1", 0);
			sessionStorage.setItem('nbTentatives1',nbTentatives);
			...
			continuerPartie = true;
			sessionStorage.setItem("pointsPerdusNiveau1", 0);
			sessionStorage.setItem("pointsGagnesNiveau1", 0);
			sessionStorage.setItem('nbTentatives1',nbTentatives);
...
function lecteurAudio(){
	...
	if(oAudio.canPlayType("audio/mp3"))
		oAudio.setAttribute("src", "http://localhost/poesifacile/jeux/PlusOuMoins/media/musique1.mp3");
		else
			return;
