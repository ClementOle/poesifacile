...
//On vide le localStorage qui permet ensuite d'y stocker les points
...
form.addEventListener('submit', function(event) {
    ...
    if (verifLigneColonne() && verifChiffres()) {
        ...
        reset.classList.add('disparition');
...
buttonSuite.addEventListener('click', function (event) {
    event.preventDefault();
    event.stopPropagation();
    sessionStorage.setItem("pointJeux", point);
...
function affichageRes(res) {
    ...
    if (res == true) {
        let point = calculPoints(minute, nbErreur);
